...
public class PubShopById extends Action {
	@Override
	public void afterExcute(HttpServletRequest arg0, HttpServletResponse arg1)
		...
		//String[] r = ids.split(",");
		// 生成店铺
		String shopId = this.oasession.getCompanyCode();
		...
		if (boService.loadInterceptor()) {
			...
			JSONArray jsonArray = JSONArray.fromObject(context.get("workItem"));
			// System.out.print(jsonArray.toString());
		} else {
			arg0.setAttribute("actionForward_", null);
			this.callURI("/msg/msgorder.jsp", arg0, arg1);
		}
		this.callURI("", arg0, arg1);

		// 生成店铺
		/*
		 * List productList = productListDaoImpl.listProductsByShop(shopId); for
		 * (int j = 0; j < productList.size(); j++) { Map map = (Map)
		 ...
		 * 
		 * } Logger.getLogger(this.getClass().getName()).info( "save product..."
		 * + id + ".html"); // beans = new ArrayList<FormItemBean>(); //
		 * formItemBean = new FormItemBean(); //
		 * formItemBean.setItemName("name"); // formItemBean.setType("0"); //
		 * beans.add(formItemBean); // formItemBean = new FormItemBean(); //
		 * formItemBean.setItemName("content"); // formItemBean.setType("3"); //
		 * beans.add(formItemBean); // String fid; // String id =
		 * arg0.getParameter("id"); if (id == null) id = ""; fid = "dish";
		 * boService.init("dish", null, null); if (oasession == null) {
		 * oasession = new OASession(); oasession.setLoginID(0);
		 * oasession.setUserName("guest"); } boService.setSession((OASession)
		 * oasession); // String templName =
		 * boService.getFormBean().getTemplFile(); // LazyDynaBean bean = null;
		 * // 获得模 if (!templName.equals("-")) { // bean = boService.load("name",
		 * templName, "eformtempl", // Beans); } context = new
		 * VelocityContext(); // context.put("type1Name", type1Name); //
		 * context.put("type2Name", type2Name); // context.put("type3Name",
		 * type3Name); // context.put("type1Id", type1Id_); //
		 * context.put("type2Id", type2Id_); // context.put("type3Id", type); //
		 * String templName = boService.getFormBean().getTemplFile(); //
		 * HashMap<String,?> Varables=new HashMap(); // arg0.getp
		 * boService.setId(id); if (boService.loadInterceptor()) {
		 * boService.load(id, arg0, context, fid); context.put("htmlFile",
		 * "/eform/" + "dishseehi_m.html"); String layoutAdapter =
		 * this.getActionBean() .getLayoutAdapter(); if (layoutAdapter != null)
		 * { context.put("layoutAdapter", "/eform/layoutadapter/" +
		 ...
		 * arg0.getRealPath(""); // ar // arg0.getPathTranslated(); VelocityUtil
		 * velocityUtil = new VelocityUtil(context); context.put("androidCode",
		 * boService.getMainBeans()); String title =
		 * this.getActionBean().getTitle(); context.put("title", title); String
		 * location = this.getActionBean().getLocation();
		 ...
		 * velocityUtil.code("/eform/" + param2, contextPath); File file_ = new
		 * File(mContextPath + "/fancompany/" + shopId); if (!file_.exists())
		 * file_.mkdir(); String file = mContextPath + "/fancompany/" + shopId +
		 * "/" + id + ".html"; FileWriter fw = new FileWriter(file); //
		 * BufferedWriter bw = new BufferedWriter(fw); // utf-8 BufferedWriter
		 * bw = new BufferedWriter( new OutputStreamWriter(new FileOutputStream(
		 * file), Charset.forName("UTF-8"))); // bw.newLine();
		 * bw.write(content); bw.close(); fw.close(); } JSONArray jsonArray =
