...
public class ExportCompany extends Action {
	@Override
	public void afterExcute(HttpServletRequest arg0, HttpServletResponse arg1)
		...
		for (Integer i = 0; i < a.length; i++) {
			...
			boService.getParameterValues().put("address", ai[1].trim());
			boService.getParameterValues().put("tel", ai[2].trim());
	...
	@Test
	public void a() {
		String name = "½­ÄÏ¹ÖÎ¶Ñ¼   a98    -              Æ®ÏãÆ¡¾ÆÊ¯¼¦   a68"
				+ "-Ã÷Â¯²ÝÓã    a58    -             ÎýÖ½öÔÓã       a68";

		String a1[] = name.split("-");
		for (Integer i = 0; i < a1.length; i++) {
			String a2[] = a1[i].split("a");
