...
public class ReportByOperator extends Action {
	String begin_ = null;
	String end_ = null;
	...
	public void reportBuyType(HttpServletRequest arg) throws SQLException {
		...
		for (int i = 0; i < types.size(); i++) {
			...
			String sql =
					 " (select operator,sum(CONVERT(float,money)-CONVERT(float,change)) as sum1    from fanorderbuy where createdate>'"
